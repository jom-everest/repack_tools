...
def unpack_un():
    ...
    target_file = os.path.join(src_dir, 'arc_un.tar')
...
def unpack_c():
    ...
    tar_file = os.path.join(dst_dir, 'arc_c.tar')
...
#    subprocess.run([
#        ZSTD_PATH, '-d', zst_file, '-o', tar_file
#    ])
...
def make_archive_from_uncompressible():
    ...
    files_count = move_all_files_by_ext(src_dir, target_dir, unscompressible_extensions)
